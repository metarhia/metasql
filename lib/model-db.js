...
class DatabaseModel extends schema.DomainModel {
  ...
  async getPreviousVersion() {
    ...
    return schema.loadModel(previousPath);
  }
...
const loadModel = async modelPath => {
  const entities = await schema.readModel(modelPath);
  const database = entities.get('.database');
  if (!database) {
    throw new Error(`File is not found: ${modelPath}/.database.js`);
  }
  const ModelClass = schema.implementations[database.driver];
  const databaseModel = new ModelClass(entities, modelPath);
