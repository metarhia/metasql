...
class Modify {
  constructor(db, sql, args, record) {
    this.db = db;
    this.sql = sql;
    this.record = record;
    this.args = args;
  ...
  then(resolve, reject) {
    ...
    const { returning, onConflict } = this.options;

    if (onConflict) {
      const { fields, action, excludeFields } = onConflict;
...
class Database {
  ...
  register(name, fields, params, data) {
    const sql = [
      'WITH "Reg" AS (',
      '  INSERT INTO "Identifier" ("categoryId")',
      `  VALUES ((SELECT "id" FROM "Category" WHERE "name" = '${name}'))`,
  ...
  insert(table, record) {
    ...
    return new Modify(this, sql, data, record);
  }
