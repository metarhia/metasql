...
const whereValue = (value) => {
  ...
  if (value instanceof Array) return ['= ANY', value];
  return ['=', value];
};
...
const buildWhere = (conditions, firstArgIndex = 1) => {
  ...
  for (const where of conditions) {
    ...
    for (const key of keys) {
      ...
      if (value !== undefined) {
        let par = '$' + (i++).toString();
        if (value instanceof Array) par = '(' + par + ')';
        conjunction.push(`"${key}" ${operator} ${par}`);
        args.push(value);
      }
    }
...
class Database {
  ...
  query(text, values) {
    ...
    this.console.debug(`${text}\t[${data}]`);

    const args = {
      text,
      values,
    };
    if (values) args.name = hash(text);

    return this.pool.query(args).catch((error) => {
