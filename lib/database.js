...
const buildSystemQuery = (type, fields, ...conditions) => {
  ...
  for (const where of conditions) {
    ...
    for (const key of keys) {
      const [operator, value] = whereValue(where[key]);
      clause.push(`${key} ${operator} ${value}`);
    }
  }

  const sql = [
    `SELECT ${names} FROM INFORMATION_SCHEMA.${type}`,
    clause.length ? `WHERE ${clause.join(' AND ')}` : '',
  ];
...
class Database {
  ...
  async upsert(table, record, constraint) {
    const rows = await this.select(table, constraint);

    if (rows.length) {
      return this.update(table, record, constraint);
    } else {
  ...
  async fields(table) {
    const sql = buildSystemQuery('COLUMNS', ['column_name'], {
      TABLE_NAME: `N'${table}'`,
    });
  ...
  async tables() {
    const sql = buildSystemQuery('TABLES', ['table_name'], {
      TABLE_SCHEMA: `'public'`,
    });
