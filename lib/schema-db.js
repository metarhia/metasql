...
class DatabaseSchema {
  ...
  static async load(schemaPath) {
    ...
    const database = await schema.read(dbPath);
    if (!database) throw new Error(`File is not found: ${dbPath}`);
    const typesPath = path.join(schemaPath, '.types.js');
    const types = await schema.read(typesPath);
    ...
    for (const file of files) {
      ...
      const entitySchema = await schema.read(absPath);
      if (!entitySchema) continue;
      const schemaName = path.basename(file.name, '.js');
      databaseSchema.entities.set(schemaName, entitySchema);
    }
