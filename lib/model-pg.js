...
const createMany = (entityName, { many }, model) => {
  ...
  return model.createEntity(crossName);
};
...
const primaryCustom = (entityName, fields, entity) => {
  ...
  if (entity) {
    for (let i = 0; i < idx.length; i++) {
      const field = idx[i];
      const def = entity.fields.get(field);
      if (!def) continue;
      const { type } = def;
      if (!type) continue;
...
class PgModel extends DatabaseModel {
  constructor(entities, modelPath) {
    const types = entities.get('.types');
    entities.set('.types', { ...PG_TYPES, ...types });
    super(entities, modelPath);
  }

  createEntity(name) {
    const entity = this.entities.get(name);
    const { fields, indexes } = entity;
    const sql = [];
    const idx = [];
    ...
    for (const def of fields.values()) {
      ...
      if (def.type) {
        ...
        let pgType = kind === DB_FIELD ? this.types[def.type] : 'bigint';
        ...
        if (def.length) {
          if (typeof def.length === 'number') {
            pgType = `${pgType}(${def.length})`;
          } else if (Array.isArray(def.length)) {
            pgType = `${pgType}(${def.length[1]})`;
          } else if (typeof def.length.max === 'number') {
    ...
    for (const def of indexes.values()) {
...
schema.implementations.pg = PgModel;
