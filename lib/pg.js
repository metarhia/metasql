...
const PG_TYPES = {
  ...
  array: { metadata: { pg: 'array' } },
  set: { metadata: { pg: 'array' } },
...
const createEntity = (model, name) => {
  ...
  for (const field of fields) {
    ...
    if (pgType.toLowerCase() === 'array') {
      const { pg: pgArrayType } = def.value?.constructor.metadata || null;
      if (!pgArrayType)
        throw new Error(
          `Unknown array type: ${def.value?.type} in ${name}.${field}`,
        );
      const isInt = (num, grThn = 0) => Number.isInteger(num) && num > grThn;
      const dims = def.dims || (maxLength ? [maxLength] : undefined);
      pgType = `${pgArrayType}`;
      pgType += dims?.length > 1 || isInt(dims, 1) ? '' : ' ARRAY';
      if (Array.isArray(dims)) {
        pgType += dims.map((dim) => `[${isInt(dim) ? dim : ''}]`).join('');
      } else if (isInt(dims)) {
        pgType += Array(dims).fill('[]').join('');
      }
    } else if (maxLength) pgType = `${pgType}(${maxLength})`;
