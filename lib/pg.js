...
const createIndex = (entityName, indexName, def, entity) => {
  ...
  const type = typeof fieldList;
  if (type === 'boolean') fieldNames.push(indexName);
  else fieldNames.push(...fieldList);
  const names = [];
...
const flatFields = (fields) => {
  ...
  for (const name of names) {
    ...
    if (value.type === 'schema') {
      const groupNames = Object.keys(value.schema.fields);
      for (const field of groupNames) {
        const fieldName = name + toUpperCamel(field);
        flat[fieldName] = value.schema.fields[field];
      }
    } else {
      flat[name] = value;
    }
...
const NEW_CAT = 'INSERT INTO "Category" ("id", "name", "kind") ';
const NEW_FLD = 'INSERT INTO "Field" ("id", "categoryId", "name") ';

const catId = (name) => `SELECT "id" FROM "Category" WHERE "name" = '${name}'`;
const fldId = (category, field) =>
  'SELECT "id" FROM "Field" WHERE "categoryId" = (' +
  catId(category) +
  `) AND "name" = '${field}'`;
...
const registerEntity = (model, name) => {
  ...
  const upd = [];
  ins.push(NEW_ID);
  ins.push(NEW_CAT + `VALUES (lastval(), '${name}', '${entity.kind}');`);
  upd.push(
    'UPDATE "Identifier" SET "categoryId" = (' +
      catId('Category') +
      ') WHERE "id" = (' +
      catId(name) +
      ');'
  ...
  for (const field of fields) {
    ins.push(NEW_ID);
    ins.push(NEW_FLD + `VALUES (lastval(), (${categoryId}), '${field}');`);
    upd.push(
      'UPDATE "Identifier" SET "categoryId" = (' +
        catId('Field') +
        ') WHERE "id" = (' +
