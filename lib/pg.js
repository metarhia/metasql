...
const registerEntity = (model, name) => {
  ...
  const sql = [];
  sql.push(NEW_ID);
  sql.push(NEW_CAT + `VALUES (lastval(), '${name}', '${entity.kind}');\n`);
  const flat = flatFields(entity.fields);
  const fields = Object.keys(flat);
  const categoryId = `(SELECT "id" FROM "Category" WHERE "name" = '${name}')`;
  for (const field of fields) {
    sql.push(NEW_ID);
    sql.push(NEW_FLD + `VALUES (lastval(), ${categoryId}, '${field}');\n`);
  }
  return sql.join('\n');
};
