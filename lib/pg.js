...
const createIndex = (entityName, indexName, def, entity) => {
  ...
  const indexMethods = ['btree', 'hash', 'gin', 'gist', 'spgist', 'brin'];
  const maxMethodLength = indexMethods.reduce(
  ...
  if (typeof indexFields === 'string') {
    const hasMethod = indexMethods.some((method) =>
      indexFields.substring(0, maxMethodLength).includes(method),
    );
    if (hasMethod)
      return `CREATE ${uni}INDEX ${idxName} ON "${entityName}" USING ${indexFields};`;
    return `CREATE ${uni}INDEX ${idxName} ON "${entityName}" ${indexFields};`;
  }
