...
const DIR_TABLE = ['dictionary', 'registry', 'entity'];
const DIR_AUX = ['log', 'details', 'relation'];
const DIR_DATA = ['struct', 'form', 'view', 'projection'];
const DIR_KIND = [...DIR_TABLE, ...DIR_AUX, ...DIR_DATA];
  ...
  preprocess(raw) {
    ...
    for (const key of keys) {
      ...
      if (first && isUpperCamel(key)) {
        if (short) {
          const [kind, scope = 'memory'] = value.split(' ').reverse();
          value = { scope, kind };
        }
      ...
      const def = { name: key, ...FIELD_DEF, ...fieldDef };
      if (def.length) {
