...
class SelectBuilder extends QueryBuilder {
  ...
  count(field = '*') {
    ...
    this.operations.select.add({ type: 'count', field });
    return this;
  }

  avg(field) {
    this.operations.select.add({ type: 'avg', field: this.escapeKey(field) });
    return this;
  }

  min(field) {
    this.operations.select.add({ type: 'min', field: this.escapeKey(field) });
    return this;
  }

  max(field) {
    this.operations.select.add({ type: 'max', field: this.escapeKey(field) });
    return this;
  }

  sum(field) {
    this.operations.select.add({ type: 'sum', field: this.escapeKey(field) });
    return this;
  }
  ...
  processSelect(select) {
    ...
    return mapJoinIterable(
      select,
      op => (typeof op === 'string' ? op : functionHandlers[op.type](op)),
      ', '
    );
  }
